...
import com.rihua.api.domain.Job;
...
public interface JobRepository extends JpaRepository<Job, UUID> {
     ...
     * @return 求人ページ
     */
    Page<Job> findByPosterAndDeletedAtIsNull(User poster, Pageable pageable);
     ...
     * @return 求人ページ
     */
    @Query("SELECT j FROM Job j WHERE j.remoteWork = true AND j.status = 'ACTIVE' AND j.deletedAt IS NULL")
    ...
    @Query("SELECT j FROM Job j WHERE " +
           ...
           "(:remoteWork IS NULL OR j.remoteWork = :remoteWork) AND " +
           "j.status = 'ACTIVE' AND j.deletedAt IS NULL " +
     ...
     * @return 公開求人数
     */
    @Query("SELECT COUNT(j) FROM Job j WHERE j.poster = :poster AND j.status = 'ACTIVE' AND j.deletedAt IS NULL")
    long countActiveJobsByPoster(@Param("poster") User poster);
